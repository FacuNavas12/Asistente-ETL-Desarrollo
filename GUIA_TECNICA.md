...
Referencia de arquitectura, comandos y convenciones de este proyecto.

## Dónde vive la documentación

**Un archivo = una pregunta:**

| Pregunta | Archivo |
|---|---|
| ¿Cuál es la arquitectura objetivo, capas, R1-R12? | `docs/arquitectura-objetivo.md` — el mapa capa↔directorio-actual vive ahí. Andamiaje ya escrito sin mover código todo: `backend/app/README.md` + un `README.md` por carpeta/subpaquete (capa, qué entra/sale, qué NO va ahí), y `backend/tests/test_architecture_layers.py` (test AST en modo "no empeorar" — R1/R3/R4 en un recorte acotado, ver su docstring) + `backend/tests/test_pdi_step_coherence.py` (coherencia prompt↔alias↔builder de PDI) |
| ¿Cómo decide el backend en cuántos `.ktr` fragmenta una etapa? | `docs/referencia/fragmentacion.md` |
| ¿Qué garantiza el contrato de DDL y quién lo hace cumplir? | `docs/referencia/contrato-ddl.md` |
| ¿Cuándo SCD1 vs SCD2, y qué step de Pentaho carga cada dimensión? | `docs/referencia/scd.md` |
| ¿Diverge el emisor del comportamiento real de Kettle en algún step? | `docs/referencia/kettle-comportamiento.md` |
| ¿Qué es dominio y qué es infraestructura cuando no es obvio? | Ver "Criterio de capas: dominio vs. infraestructura" acá abajo |

---

## Criterio de capas: dominio vs. infraestructura

Amplía `arquitectura-objetivo.md` (tabla de Capas, R1). Escrito acá, no ahí, porque es un ajuste al proxy que la tabla usa, no una regla nueva — ver por qué abajo.
...
app/
  ...
  domain/                          — primer paquete físico de esta capa
  ...
  services/
    ...
    etl_generator.py              — construye prompt(s), llama LLM, arma ETLGenerateResponse
                                    (etapas: list[EtapaOutput] + kjb_master;
                                    ya no ktr_xml/ktr2_xml/kjb_xml). Cada EtapaOutput trae nombre (etiqueta
                                    ...
                                    (parte en N sub-transformaciones si compute_cut() lo exige, ver `docs/referencia/fragmentacion.md`) +
    ...
    superset_client/               — integración con API REST de Superset (login, get_or_create_database,
                                    import_dashboard). FUERA DE ALCANCE del proyecto — quedó desplazado, no se lo toca ni se
