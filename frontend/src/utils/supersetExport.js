...
// ── Import + open (ETL card menu / EtlDetail comparten esta lógica) ──────────
...
/**
 * Genera el ZIP, lo importa en Superset via backend y abre el dashboard resultante.
 * Chequeo on-click (no gatea el botón): si conn_dwh está configurada y las
 * tablas DWH todavía no tienen filas reales, confirma con el usuario antes de
 * abrir en modo demo — cancelar aborta sin pegarle al backend.
 */
export async function exportEtlToSuperset(etl) {
  ...
  if (etl?.id) formData.append("etl_id", etl.id);
