...
/**
 * CRUD sobre una lista externa de items (por I­ndice).
 */
...
/**
 * Estado y operaciones del panel de construccion de tablas.
 *
...
/**
 * Tabla de display genÃ©rica para columnas.
 *
 * columnDefs: [{ key, label, render? }]
 * onRemove(index) : agrega columna de boton por fila.
        ...
        <tbody>
          {columns.map((col, i) => (
            <tr key={col.id ?? i}>
              ...
              {onRemove && (
                <td>
                  <button className="staging-remove-btn" onClick={() => onRemove(i)}>x</button>
                </td>
              )}
            </tr>
          ))}
...
/**
 * Panel de construccion de tabla con header "Nueva tabla / Editar tabla".
 */
...
/**
 * Boton inferior "Agregar tabla / Guardar cambios".
 */
...
export function TableCardHeader({ name, origen, onEdit, onRemove, prefix }) {
  return (
    <div className="dwh-table-card__header">
      ...
      {onEdit   && <button className="staging-edit-btn"   onClick={onEdit}>edit</button>}
      {onRemove && <button className="staging-remove-btn" onClick={onRemove}>remove</button>}
    </div>
  );
}
 ...
 * renderCard(table, index) â†’ contenido interno de cada dwh-table-card.
 */
