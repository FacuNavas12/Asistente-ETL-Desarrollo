    ...
    <div className="origen-col-collapse">
      <div className="origen-col-collapse__header">
        ...
        <span className="origen-col-meta">
          {col.dataType}{col.dataFormat ? ` Âaaaaaaaaaaaa· ${col.dataFormat}` : ""} Â· {col.role}
        </span>
        {hasData && (
          <span className="origen-col-count">{col.data.length} dato{col.data.length !== 1 ? "s" : ""}</span>
        )}
        {onEdit   && <button className="staging-edit-btn"   onClick={onEdit}>editar</button>}
        {onRemove && <button className="staging-remove-btn" onClick={onRemove}>eliminar</button>}
      </div>
              ...
              <div className="origen-dato-tags">
                {ed.currentCol.data.map((d, i) => (
                  <span key={i} className="origen-dato-tag">
                    {d}
                    <button className="origen-dato-remove" onClick={() => handleRemoveDato(i)}>âœ•</button>
                  </span>
                ))}
              </div>
            )}
