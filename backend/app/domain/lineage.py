"""
Reglas puras de construcción del grafo de linaje: qué step toca qué tabla,
cómo se clasifica su capa (origen/staging/dwh), cómo se cosen N archivos KTR
en un grafo continuo. Split de `services/lineage_builder.py` (O2-c).

Devuelve `LineageGraphData` (dataclasses stdlib), no `schemas.lineage.Lineage`
(Pydantic) — `domain/README.md` prohíbe un `BaseModel` acá aunque el concepto
sea el mismo; `services/lineage_builder.py` envuelve esto en el contrato de
transporte real para el borde de la API.

`_parse_ktr_xml` (lee XML ya serializado) queda en `services/lineage_builder.py`
...
class LineageGraphData:
    nodes: list[LineageNodeData]
    edges: list[LineageEdgeData]
...
def build_lineage(ktr_data: dict) -> LineageGraphData:
    """
    ...
    edges: list[LineageEdgeData] = [
        LineageEdgeData(from_step=h["from"], to_step=h["to"])
        for h in valid_hops
    ]
...
def stitch_lineage_many(ktr_data_list: list[dict]) -> LineageGraphData:
    """
...
def stitch_lineage(ktr_data_1: dict, ktr_data_2: dict) -> LineageGraphData:
    """Caso M=2 de stitch_lineage_many() — origen→STG (KTR_1) y STG→DWH
    (KTR_2). Firma preservada por compatibilidad: la usa
    services.lineage_builder.stitch_lineage_from_xml (routers/ai.py, endpoint
    público /api/ai/lineage-from-ktr que re-deriva linaje a partir de 2 XML
    ya servidos)."""
