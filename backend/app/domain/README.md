...
| `lineage.py` | `build_lineage()`/`stitch_lineage_many()`/`stitch_lineage()` — grafo de linaje puro sobre el dict KTR (split de `services/lineage_builder.py`, O2-c). Devuelve `LineageGraphData` (dataclass propia, stdlib), no `schemas.lineage.Lineage` — ver "Qué NO va acá". `services/lineage_builder.py` envuelve el resultado en `Lineage` (Pydantic) para el borde de la API y conserva `_parse_ktr_xml` (infra, lee XML ya serializado). |
