...
**Resultado 2026-08-03 (primera corrida real):** `55 failed, 688 passed` (`backend/venv/Scripts/python.exe -m pytest tests/ -q`). 54 coinciden con el set histórico ya documentado en `ESTADO.md` F4 (36 `test_api.py` + 6 `test_ktr_build_job_api.py` — sin servidor `uvicorn` vivo; 11 `test_structured_outputs.py` — cuota Gemini agotada; 1 `test_ktr_xml_validator.py` — ya roto antes de esta sesión). El número de referencia de este archivo (45) estaba desactualizado frente a `ESTADO.md` (54); se corrige acá.

**1 fallo nuevo, no preexistente:** `tests/test_dimension_field_repair.py::test_repair_dimension_loader_fields_floor_when_gate_fails` — el test espera que, cuando el repair propone un `stream_field` alucinado (no existe en el stream real), el gate lo rechace y sobreviva el finding `severity="error"` original. El código hoy no rechaza — degrada a `tipo="info"` con mensaje de "mapeo inferido automáticamente", aceptando el campo alucinado sin gate real. Es un hallazgo de lógica en `_repair_dimension_loader_fields`/`enforce_dimension_step_policy` (serie fk-categoria), fuera de alcance de O0 (nada de lógica) — no se toca acá. Va a `01-hallazgos.md` o al backlog de O1 según corresponda cuando esa sesión lo tome.
