...
- El corte se basa en una matriz step × tabla × {lee, escribe}: toda tabla que aparezca como W y R dentro de la misma etapa marca una frontera de corte; los componentes conexos resultantes son los KTR; el orden entre componentes sale del grafo de FK.
- Antes de emitir los artefactos, un set de validadores guardarraíl (V1: ninguna tabla W y R en el mismo KTR; V2: toda tabla leída por un lookup tiene productor en el job; V3: un solo escritor por tabla por KTR) debe pasar. Si falla, el sistema aborta con mensaje preciso — nunca emite un artefacto incoherente (D5, R11).
