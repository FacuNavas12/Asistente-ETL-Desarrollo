...
| D17 | F2 aprobado por el usuario | Archivado → [`02b-decisiones-cerradas.md`](02b-decisiones-cerradas.md) |
| D18 | H2 (config string→object) pospuesto | No decidido — requiere spike |
| D19 | Wiring de servicio cerrado, HTTP en notificación | Archivado → [`02b-decisiones-cerradas.md`](02b-decisiones-cerradas.md) |
| D20 | Forma de la respuesta con N archivos | Archivado → [`02b-decisiones-cerradas.md`](02b-decisiones-cerradas.md) |
...
| D24 | Track A retomada, A0 ejecutada | Archivado → [`02b-decisiones-cerradas.md`](02b-decisiones-cerradas.md) |
| D25 | A0.5 ejecutada, deriva H29 | Archivado → [`02b-decisiones-cerradas.md`](02b-decisiones-cerradas.md) |
...
| D28 | D20-punto5 cerrado: frontend consume `etapas`/`kjb_master`; linaje recalculado se borra del front; datos viejos se rechazan explícitos; `EtapaOutput` gana `nombre`; Superset fuera de alcance | Archivado → [`02b-decisiones-cerradas.md`](02b-decisiones-cerradas.md) |
...
| D59 | O1, lote E-04…E-08: StringOperations/Unique/ExcelInput/JsonInput/TextFileOutput corregidos contra `readData()` real de Kettle, evidencia citada por clase+línea | Archivado → [`02b-decisiones-cerradas.md`](02b-decisiones-cerradas.md) |
...
| D61 | O2-a: `common.py` partido — `_yn`/`KtrBuilderError` quedan (dominio puro), `_sub` se muda a `xml_helpers.py` nuevo (infra) | Archivado → [`02b-decisiones-cerradas.md`](02b-decisiones-cerradas.md) |
| D62 | O2-b (T1): `resolve_step_table()` nueva en `domain/step_table.py` — unifica el `if not table: continue` mudo de `fragmentation.py`/`dimension_step_policy.py`/`fields_validate.py`, devuelve mensaje en vez de tragarlo | Archivado → [`02b-decisiones-cerradas.md`](02b-decisiones-cerradas.md) |
| D63 | O1-b: dedupe de vocabulario cruzado en `DimensionLookup` — canal único `check_dimension_lookup_fields`, registro retroactivo de D60 | Archivado → [`02b-decisiones-cerradas.md`](02b-decisiones-cerradas.md) |
| D64 | O1: Alcance punto 3 de `10-estabilizar-emision.md` (findings de `enforce_dimension_step_policy` llegan al usuario) — verificado en corrida real contra el corpus de E-01, canal ya existía, sin cambio de código. Encontrado E-20 (duplicación de `PRE_EMIT_PASSES`) | Archivado → [`02b-decisiones-cerradas.md`](02b-decisiones-cerradas.md); E-20 abierto en `errores.md`, no bloquea |
| D65 | O1-b cierra vía corrida real async (`/generate-async`→`/status`) + fix de Bloque 3 (`_build_response_from_two_ktr_data` ya no descarta la etapa origen→STG cuando STG→DWH falla estructuralmente). Con esto, O1 completo | Archivado → [`02b-decisiones-cerradas.md`](02b-decisiones-cerradas.md), commit `80a1e3b` |
| D66 | O2-c: `lineage_builder.py` partido — `build_lineage`/`stitch_lineage_many`/`stitch_lineage` a `domain/lineage.py` (`LineageGraphData`, dataclass stdlib); `_parse_ktr_xml` queda de infra en `services/lineage_builder.py`. Registro retroactivo — código y mapa ya decían "Ejecutado" sin D-N propia | Archivado → [`02b-decisiones-cerradas.md`](02b-decisiones-cerradas.md) |
| D67 | O2 verificado completo al pedir "empezar O2": O2-a/b/c ya ejecutadas (D61/D62/D66), `test_architecture_layers.py` verde, `FROZEN_R1` vacío. Nada nuevo que ejecutar — corrección de `docs/README.md` (línea O2-c apuntaba a D63, que ya era de otro tema) | Archivado → [`02b-decisiones-cerradas.md`](02b-decisiones-cerradas.md) |
...
| D69 | O2-d: el borde de un upload vive en infraestructura, no en el router — `routers/schema.py::infer_schema` manejaba tempfile/límite/cleanup directo, `services/file_schema.py::infer_schema_from_upload()` pasa a ser dueño de eso | Archivado → [`02b-decisiones-cerradas.md`](02b-decisiones-cerradas.md) |
