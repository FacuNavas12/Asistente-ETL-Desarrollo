...
Track F: F1 ✓ (2026-07-22)
    ...
    F2.5 ✓ (código, 2026-07-24) — soporte JobEntryJob (H7) cerrado
         v
    F3 — implementación (corte + jerarquía de jobs + V1/V2/V3) — F1.5/F2.5/F2 ya no bloquean. D16 camino 1 parcial en código (scd_type==2 cubierto, 0/1 residual, ver 02-decisiones.md) — no bloquea, sigue como caso conocido. EN CURSO: algoritmo puro + wiring de servicio (split/_build_ktr_stage/_build_job_plan N-ario/stitch_lineage_many) cerrados en código (D19); falta extender ETLGenerateResponse/frontend para aplicar el corte de verdad en el flujo HTTP.
```
...
- **NO hecho — bloquea considerar F3 "terminada":**
  1. **Extender `ETLGenerateResponse`/frontend** para entregar N archivos por HTTP (hueco de arriba, D19) — recién ahí el flujo en vivo reemplaza las llamadas directas a `build_ktr()` por `_build_ktr_stage()` y deja de limitar cada etapa a 1 archivo.
  2. **Test de integración end-to-end contra el pipeline HTTP completo** con un caso que dispare un corte real (hoy los tests de wiring llaman las funciones de servicio directamente — `_build_ktr_stage`/`_build_job_plan`/`stitch_lineage_many` — no pasan por `generate_etl_from_inference`).
  3. **D13 (definición de terminado):** registro de deltas + actualización de `CLAUDE.md`/progreso — prematuro mientras 1-2 sigan abiertos.

**Para la próxima sesión: decidir el diseño de la extensión de `ETLGenerateResponse` (¿lista de KTRs en vez de slots fijos `ktr_xml`/`ktr2_xml`? ¿cómo lo consume el ZIP del frontend?) — es lo que desbloquea aplicar el corte de verdad en el flujo HTTP en vez de solo notificarlo.**
...
| F5 — Limpieza de bajo costo, sin dependencias | **Dedup de `_parse_cfg` movido a F1.5 (2026-07-22, ver D14)** — F5 ya no lo incluye, evita que dos fases reclamen el mismo fix. Queda: fix docstring `etl_output.py` (H12), `s["name"]` sin `.get()` en `validate.py` | Ninguna — "cero riesgo" según el propio material de origen | H12 |
