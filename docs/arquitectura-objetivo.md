...
| `services/validate_business_rules.py` | `domain/` | **No estaba en la hipótesis original** (quedaba implícito en el bucket genérico `services/`). Verificado: cero imports más allá de `__future__`. Es un stub (D23, `docs/refactor/02-decisiones.md`) — la clasificación puede cambiar si su implementación real termina necesitando orquestar algo fuera de dominio. |
| `services/ktr_builder/common.py` | **partido** | `_yn` (`:14-28`) y `KtrBuilderError` (`:31-36`) son puros → `domain/`. `_sub` (`:7-11`) arma un `xml.etree.ElementTree.Element` → `infrastructure/pentaho/`. Mismo archivo, dos capas, sin que el nombre lo avise. |
...
| `services/lineage_builder.py` | **partido** | `build_lineage` (`:84-129`), `stitch_lineage_many` (`:132-220`), `stitch_lineage` (`:223-228`) son puros sobre el dict KTR → `domain/`. `_parse_ktr_xml` (`:231-256`) lee XML ya serializado (camino inverso, solo para `/api/ai/lineage-from-ktr`) → `infrastructure/pentaho/`. |
